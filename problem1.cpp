...
vector<string> v = {"I", "we", "ar", "wear", "arm", "army", "my", "shoe", "shoes"};


bool isValid(string s, vector<string> v)
{
...
void findWord(string input, int L, int R, string current);

int main()
{
    string x = "Iwearmyshoes", s = "";
    findWord(x, 0, x.size(), s);
}

void findWord(string input, int L, int R, string current)
{
    if (isValid(current, v))
    {
        cout << current << " ";
    }
    else
    {
        ...
        {
            current += input[i];
            // Error below in the next line
            findWord(input.substr(i, R - i), L + 1, R, current);
            current.pop_back();
        }
    }
}
